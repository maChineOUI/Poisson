...
/* ** argv: Values of arguments */
{
  ...
  nbpoints=10000;      /* Total discretization points */
  ...
  write_vec(RHS, &la, "/app/results/RHS.dat");
  write_vec(EX_SOL, &la, "/app/results/EX_SOL.dat");
  write_vec(X, &la, "/app/results/X_grid.dat");
  ...
  write_GB_operator_colMajor_poisson1D(AB, &lab, &la, "/app/results/AB.dat");
  ...
  // For Jacobi/GS preconditioners, no extra kv is needed
  kv = 0;
  ku = 1;
  kl = 1;
  lab = kv + kl + ku + 1;   // = 3  (keep consistent with AB)
  ...
  if (IMPLEM == JAC || IMPLEM == GS) {
    write_GB_operator_colMajor_poisson1D(MB, &lab, &la, "/app/results/MB.dat");
    richardson_MB(AB, RHS, SOL, MB, &lab, &la, &ku, &kl, &tol, &maxit, resvec, &nbite);
  }

  /* Write solution and convergence history to files */
  write_vec(SOL, &la, "/app/results/SOL.dat");              /* Final solution */

  /* Write convergence history */
  printf("\nMethod = %d, nbite = %d, final relres = %.6e, tol = %.1e, maxit = %d\n",
       IMPLEM, nbite, resvec[nbite-1], tol, maxit);

  write_vec(resvec, &nbite, "/app/results/RESVEC.dat");     /* Residual norm at each iteration */
